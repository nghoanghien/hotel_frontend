...
import {
  ...
  Calendar, CreditCard, Hash, User, ShoppingBag, Phone, MapPin, AlertCircle, BedDouble, Layers, Settings, Clock, AlertTriangle
} from '@repo/ui/icons';
...
const COLUMN_GROUPS = {
  general: {
    ...
    columns: [
      { key: 'roomNumber', label: 'Room Number', icon: <Hash className="w-3 h-3" /> },
      { key: 'type', label: 'Room Type', icon: <LayoutGrid className="w-3 h-3" /> },
      { key: 'status', label: 'Status', icon: <CheckCircle className="w-3 h-3" /> },
      { key: 'floor', label: 'Floor', icon: <Layers className="w-3 h-3" /> }
    ]
  },
  financial: {
    label: 'Financial',
    ...
    columns: [
      { key: 'basePrice', label: 'Base Price', icon: <CreditCard className="w-3 h-3" /> },
      { key: 'maxOccupancy', label: 'Max Occupancy', icon: <User className="w-3 h-3" /> }
    ]
  },
  other: {
    label: 'Other Info',
    ...
    columns: [
      ...
      { key: 'sizeInSquareMeters', label: 'Size (m²)', icon: <LayoutGrid className="w-3 h-3" /> }
    ]
  }
};

interface RoomExportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const RoomExportModal: React.FC<RoomExportModalProps> = ({
  isOpen,
  onClose,
  onExport,
  previewData = []
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      showNotification({
        ...
        format: 'Your room list has been exported successfully.'
      });
      onClose();
    } catch (error) {
  ...
  const formatCell = (item: RoomDto, key: string) => {
    const val = item[key as keyof RoomDto];
    if (key === 'basePrice') return new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(Number(val));
    if (key === 'status') {
      const status = val as string;
      const config = {
        Available: { class: 'bg-lime-100 text-lime-700' },
        Occupied: { class: 'bg-blue-100 text-blue-700' },
        Cleaning: { class: 'bg-orange-100 text-orange-700' },
        Maintenance: { class: 'bg-red-100 text-red-700' },
        OutOfOrder: { class: 'bg-gray-100 text-gray-600' },
    ...
    if (key === 'roomNumber') return <span className="font-mono font-bold">{val as string}</span>;
              ...
              <div className="bg-white rounded-[36px] p-0 shadow-2xl border border-white/20 ring-1 ring-black/5 flex flex-col md:flex-row relative overflow-hidden h-[90vh]">
                ...
                <div className="w-full md:w-[30%] bg-gray-100 p-6 flex flex-col overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 mb-6 shrink-0">
                    ...
                    <h2 className="text-2xl font-anton font-bold text-[#1A1A1A]">EXPORT ROOMS</h2>
                  </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3 px-2">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <span className={`text-sm font-bold transition-colors ${scope === 'all' ? 'text-gray-900' : 'text-gray-600 group-hover:text-gray-900'}`}>All Rooms</span>
                              {scope === 'all' ? (
                                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                                  <div className="w-5 h-5 rounded-full bg-lime-500 flex items-center justify-center shadow-sm">
                                    <Check className="w-3 h-3 text-white" strokeWidth={3} />
                            ...
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">
                              Export your complete room list without any filters applied.
                            </p>
                          </div>
                        </motion.div>
                      </div>
