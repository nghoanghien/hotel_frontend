...
const StaffSearchPopup: React.FC<StaffSearchPopupProps> = ({
  ...
  const searchFieldsConfig: SearchFieldConfig[] = [
    { key: 'query', label: 'Search', icon: Search, placeholder: placeholders?.query || 'Search Staff Name, Email...' },
  ];
          ...
          <div className="relative bg-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] 
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-anton font-bold text-[#1A1A1A] flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-lime-50 text-gray-600 flex items-center justify-center border border-lime-100">
              ...
              {searchFieldsConfig.map((field) => (
                <div key={field.key} className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:border-gray-200 focus:bg-white rounded-2xl py-3.5 pl-11 pr-4 text-sm font-medium text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-4 focus:ring-lime-500/20 focus:border-lime-500 transition-all shadow-sm"
                  />
                </div>
              ))}
            </div>
