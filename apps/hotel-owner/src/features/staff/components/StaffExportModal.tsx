...
import {
  ...
  User, Phone, Shield, Mail, AlertCircle
} from '@repo/ui/icons';
import { UserDto } from '@repo/types';

// Define Column Groups Configuration for STAFF
const COLUMN_GROUPS = {
  basic: {
    label: 'Identity',
    icon: <User className="w-4 h-4" />,
    columns: [
      { key: 'firstName', label: 'First Name', icon: <User className="w-3 h-3" /> },
      { key: 'lastName', label: 'Last Name', icon: <User className="w-3 h-3" /> },
      { key: 'email', label: 'Email Address', icon: <Mail className="w-3 h-3" /> },
    ]
  },
  details: {
    label: 'Role & Contact',
    ...
    columns: [
      { key: 'role', label: 'Assigned Role', icon: <Shield className="w-3 h-3" /> },
      { key: 'phoneNumber', label: 'Phone Number', icon: <Phone className="w-3 h-3" /> },
      { key: 'status', label: 'Account Status', icon: <CheckCircle className="w-3 h-3" /> }
    ]
  }
};
...
const StaffExportModal: React.FC<StaffExportModalProps> = ({
  isOpen,
  onClose,
  onExport,
  previewData = []
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      showNotification({
        ...
        format: 'Your staff list has been exported successfully.'
      });
      onClose();
    } catch (error) {
  ...
  const formatCell = (item: UserDto, key: string) => {
    if (key === 'status') {
      const config = {
        Active: { class: 'bg-green-100 text-green-700' },
        Inactive: { class: 'bg-gray-100 text-gray-600' },
        Suspended: { class: 'bg-red-100 text-red-700' }
      }[item.status || 'Active'] || { class: 'bg-gray-100 text-gray-600' };
    ...
    if (key === 'role') {
      ...
      return (
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${isReceptionist ? 'bg-lime-50 text-lime-700' : 'bg-gray-50 text-gray-600'}`}>
          {item.role}
        </span>
      )
    }
              ...
              <div className="bg-white rounded-[36px] p-0 shadow-2xl border border-white/20 ring-1 ring-black/5 flex flex-col md:flex-row relative overflow-hidden h-[90vh]">
                ...
                <div className="w-full md:w-[30%] bg-gray-100 p-6 flex flex-col overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 mb-6 shrink-0">
                    ...
                    <h2 className="text-2xl font-anton font-bold text-[#1A1A1A]">EXPORT STAFF</h2>
                  </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3 px-2">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <span className={`text-sm font-bold transition-colors ${scope === 'all' ? 'text-gray-900' : 'text-gray-600 group-hover:text-gray-900'}`}>All Staff Records</span>
                              {scope === 'all' ? (
                                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                                  <div className="w-5 h-5 rounded-full bg-lime-500 flex items-center justify-center shadow-sm">
                                    <Check className="w-3 h-3 text-white" strokeWidth={3} />
                            ...
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">
                              Export your complete staff list without any filters applied.
                            </p>
                          </div>
                        </motion.div>
                      </div>
                          ...
                          <table className="w-full min-w-full divide-y divide-gray-200 text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-100 bg-white">
                              {previewData.length > 0 ? (
                                ...
                                <tr>
                                  <td colSpan={activeColumnsList.length} className="px-6 py-12 text-center text-gray-400 font-medium">
