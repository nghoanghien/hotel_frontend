...
// Define Column Groups Configuration for BOOKINGS
const COLUMN_GROUPS = {
  booking: {
    label: 'Booking Info',
    icon: <FileText className="w-4 h-4" />,
    columns: [
      { key: 'bookingCode', label: 'Booking Code', icon: <Hash className="w-3 h-3" /> },
  ...
  guest: {
    ...
    columns: [
      { key: 'guestName', label: 'Guest Name', icon: <User className="w-3 h-3" /> },
      { key: 'guestPhone', label: 'Phone Number', icon: <Phone className="w-3 h-3" /> },
    ]
  },
  stay: {
    label: 'Stay Details',
    ...
    columns: [
      { key: 'roomNumber', label: 'Room Number', icon: <Key className="w-3 h-3" /> },
      { key: 'roomType', label: 'Room Type', icon: <LayoutGrid className="w-3 h-3" /> },
      { key: 'checkInDate', label: 'Check-In', icon: <Calendar className="w-3 h-3" /> },
      { key: 'checkOutDate', label: 'Check-Out', icon: <Calendar className="w-3 h-3" /> },
    ]
  },
  financial: {
    label: 'Financial',
    ...
    columns: [
      ...
      { key: 'depositAmount', label: 'Deposit', icon: <CreditCard className="w-3 h-3" /> },
      { key: 'paymentStatus', label: 'Payment Status', icon: <CheckCircle className="w-3 h-3" /> }
    ]
  }
};
...
const BookingExportModal: React.FC<BookingExportModalProps> = ({
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      showNotification({
        ...
        format: 'Your booking list has been exported successfully.'
      });
      onClose();
    } catch (error) {
  ...
  const formatCell = (item: BookingDto, key: string) => {
    // Custom getters for nested properties
    if (key === 'guestName') return <span className="font-medium text-gray-900">{item.guest?.fullName || item.guestName || 'Unknown'}</span>;
    if (key === 'guestPhone') return item.guest?.phoneNumber || item.guestPhoneNumber || '-';

    // Date formatting
    if (key === 'checkInDate' || key === 'checkOutDate') {
      const dateVal = item[key as keyof BookingDto];
      return new Date(dateVal as string).toLocaleDateString('vi-VN');
    }

    // Currency formatting
    if (key === 'totalAmount' || key === 'depositAmount') {
      const val = item[key as keyof BookingDto];
    ...
    if (key === 'status') {
      ...
      const config = {
        ...
        Confirmed: { class: 'bg-blue-100 text-blue-700' },
        CheckedIn: { class: 'bg-lime-100 text-lime-700' },
        CheckedOut: { class: 'bg-gray-100 text-gray-600' },
        Cancelled: { class: 'bg-red-100 text-red-700' },
        NoShow: { class: 'bg-purple-100 text-purple-700' },
        Refunded: { class: 'bg-orange-100 text-orange-700' },
      }[status] || { class: 'bg-gray-100 text-gray-700' };

      return (
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${config.class}`}>
          {status}
        </span>
      );
    }
    ...
    if (key === 'paymentStatus') {
      ...
      const colorClass = status === 'Paid' ? 'text-green-600 bg-green-50' : (status === 'Partial' ? 'text-orange-600 bg-orange-50' : 'text-red-600 bg-red-50');
      return (
    ...
    if (key === 'bookingCode') return <span className="font-mono font-bold">{item.bookingCode}</span>;
              ...
              <div className="bg-white rounded-[36px] p-0 shadow-2xl border border-white/20 ring-1 ring-black/5 flex flex-col md:flex-row relative overflow-hidden h-[90vh]">
                ...
                <div className="w-full md:w-[30%] bg-gray-100 p-6 flex flex-col overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 mb-6 shrink-0">
                    ...
                    <h2 className="text-2xl font-anton font-bold text-[#1A1A1A]">EXPORT BOOKINGS</h2>
                  </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3 px-2">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <span className={`text-sm font-bold transition-colors ${scope === 'all' ? 'text-gray-900' : 'text-gray-600 group-hover:text-gray-900'}`}>All Bookings</span>
                              {scope === 'all' ? (
                                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                                  <div className="w-5 h-5 rounded-full bg-lime-500 flex items-center justify-center shadow-sm">
                                    <Check className="w-3 h-3 text-white" strokeWidth={3} />
                            ...
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">
                              Export your complete booking history without any filters applied.
                            </p>
                          </div>
                        </motion.div>
                      </div>
