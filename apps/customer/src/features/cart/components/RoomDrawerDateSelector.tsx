...
interface RoomDrawerDateSelectorProps {
  open: boolean;
  onClose: () => void;
  value: DateRange;
...
export default function RoomDrawerDateSelector({ open, onClose, value, onChange }: RoomDrawerDateSelectorProps) {
              ...
              <div key={day} className="relative">
                ...
                <button
                  ...
                  className={`relative aspect-square w-full flex items-center justify-center text-sm font-medium transition-all ${isPastDate
                    ...
                    : isStart || isEnd
                      ? 'bg-[var(--primary)] text-white rounded-full z-10 shadow-md shadow-[var(--primary)]/30 scale-110'
                      : inRange
                        ? 'text-[var(--primary)]'
                        : 'hover:bg-gray-100 text-gray-700 hover:text-gray-900 rounded-full hover:scale-105'
                    }`}
                >
                  {day}
                </button>
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[90] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-[32px] shadow-2xl p-8 w-[95vw] md:w-[900px] max-w-5xl border border-gray-100 overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-4">
              <button
                onClick={prevMonth}
                className="absolute left-4 top-1/2 md:static md:mt-10 md:mr-4 w-10 h-10 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center hover:bg-white hover:shadow-md transition-all text-gray-600 hover:text-[var(--primary)]"
              >
                <ChevronLeft className="w-5 h-5" />
              </button>

              <div className="flex flex-col md:flex-row gap-8 w-full md:px-8">
                {renderMonth(0)}
              ...
              <button
                onClick={nextMonth}
                className="absolute right-4 top-1/2 md:static md:mt-10 md:ml-4 w-10 h-10 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center hover:bg-white hover:shadow-md transition-all text-gray-600 hover:text-[var(--primary)]"
              >
