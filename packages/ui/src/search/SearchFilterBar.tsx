...
export type SearchFilterBarProps = {
  ...
  title?: string;
  subtitle?: string;
  primaryColor?: string; // e.g., 'blue', 'sky', 'indigo'
  secondaryColor?: string; // e.g., 'indigo', 'blue'
};
...
const SearchFilterBar = ({ searchFields = {}, handleSearchChange, clearSearchFields, onSearch, searchFieldsConfig = [{ key: 'fullName', label: 'Họ tên', icon: User, placeholder: 'Tìm theo họ tên...' }, { key: 'idNumber', label: 'Số CMND/CCCD', icon: FileText, placeholder: 'Tìm theo CCCD/CMND...' }, { key: 'email', label: 'Email', icon: Mail, placeholder: 'Tìm theo email...' }, { key: 'phone', label: 'Số điện thoại', icon: Phone, placeholder: 'Tìm theo số điện thoại...' }], className = '', title = 'Tìm kiếm', subtitle = 'Tìm kiếm' }: SearchFilterBarProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className={`relative bg-gradient-to-br from-green-50/80 to-white rounded-2xl shadow-[0_4px_24px_rgba(34,197,94,0.08)] mb-0 border border-green-100 overflow-hidden ${className}`}>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden"><div className="opacity-[0.09] text-green-400 transform scale-[2] -translate-y-8 translate-x-20"><Search size={50} strokeWidth={3.2} /></div></div>
      ...
      <motion.div className="p-6 cursor-pointer relative z-10" onClick={toggleExpanded} whileHover={{ scale: 1.005 }} whileTap={{ scale: 0.995 }} transition={{ duration: 0.15 }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <motion.div animate={{ scale: [1, 1.15, 1], rotate: isExpanded ? 180 : 0 }} transition={{ scale: { repeat: Infinity, duration: 2, repeatType: "loop" }, rotate: { duration: 0.3 } }} className="bg-gradient-to-r from-green-500 to-lime-600 p-2.5 rounded-xl shadow-[0_4px_16px_rgba(34,197,94,0.18)]"><Search size={20} className="text-white" /></motion.div>
            <div className={`${isExpanded ? 'hidden md:block' : ''}`}>
              <h3 className="text-base font-semibold text-green-800">{isExpanded ? title : "Nhấp để tìm kiếm"}</h3>
              <motion.p initial={false} animate={{ opacity: isExpanded ? 1 : 0.7, y: isExpanded ? 0 : 2 }} transition={{ duration: 0.3 }} className="text-sm text-green-600 mt-0.5">{isExpanded ? subtitle : "Mở rộng để sử dụng bộ lọc tìm kiếm"}</motion.p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            {isExpanded && (
              <>
                <motion.button initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }} onClick={(e) => { e.stopPropagation(); handleSearchSubmit(); }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="text-sm font-medium bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-4 py-2 rounded-xl flex items-center transition-all duration-200 border border-emerald-200 shadow-sm"><Search size={16} className="mr-2" />Tìm kiếm</motion.button>
                  ...
                  return (
                    <motion.div key={field.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.4, delay: 0.1 + index * 0.1 }}>
                      <div className="relative group">
                        <input type="text" placeholder={field.placeholder || ""} value={localSearchFields[field.key] || ""} onChange={(e) => handleLocalSearchChange(field.key, e.target.value)} onKeyPress={(e) => { if (e.key === 'Enter') { handleSearchSubmit(); } }} className="w-full pl-12 pr-4 py-3.5 text-sm bg-white border border-green-100 rounded-xl shadow-sm focus:shadow-md focus:border-green-300 focus:ring-2 focus:ring-green-200 focus:ring-opacity-50 transition-all duration-200" />
                        <motion.div animate={{ scale: [1, 1.15, 1] }} transition={{ repeat: Infinity, duration: 2, repeatType: "loop", delay: index * 0.3 }} className="absolute left-3 top-3 bg-green-50 p-1.5 rounded-lg text-green-600 group-focus-within:bg-green-100 transition-colors duration-200"><IconComponent size={18} /></motion.div>
                      </div>
                    </motion.div>
                  );
                })}
